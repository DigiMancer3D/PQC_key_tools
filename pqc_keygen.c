...
int main(void) {
    srand(time(NULL));
    OQS_randombytes_switch_algorithm("system");
    printf("========== PQC Keychain Generator (SPX-QEC driven) ==========\n\n");
    ...
    printf("[5/7] Generating Master keypairs (Falcon-512 + ML-DSA-65 + SLH-DSA-SHA2-128s)...\n");
    unsigned char *f_pk, *f_sk, *d_pk, *d_sk, *s_pk, *s_sk;
    size_t f_pk_len, f_sk_len, d_pk_len, d_sk_len, s_pk_len, s_sk_len;
    ...
    printf("[6/7] Generating 9 Role keypairs...\n");
    ...
    size_t role_f_len[ROLE_COUNT], role_d_len[ROLE_COUNT], role_s_len[ROLE_COUNT];

    for (int r = 0; r < ROLE_COUNT; r++) {
        generate_keypair_set("Falcon-512", &role_f_pk[r], &role_f_len[r], &role_f_sk[r], &role_f_len[r]);
        generate_keypair_set("ML-DSA-65", &role_d_pk[r], &role_d_len[r], &role_d_sk[r], &role_d_len[r]);
        generate_keypair_set("SLH_DSA_PURE_SHA2_128S", &role_s_pk[r], &role_s_len[r], &role_s_sk[r], &role_s_len[r]);
        printf(" Role %d generated\n", r);
    }
    printf("\n");
    ...
    for (int role = 0; role < ROLE_COUNT; role++) {
        ...
        char *f_pk_h = bytes_to_hex(role_f_pk[role], role_f_len[role]);
        char *f_sk_h = bytes_to_hex(role_f_sk[role], role_f_len[role]);
        char *d_pk_h = bytes_to_hex(role_d_pk[role], role_d_len[role]);
        char *d_sk_h = bytes_to_hex(role_d_sk[role], role_d_len[role]);
        char *s_pk_h = bytes_to_hex(role_s_pk[role], role_s_len[role]);
        char *s_sk_h = bytes_to_hex(role_s_sk[role], role_s_len[role]);
    ...
    json_object_set_new(root, "algorithm", json_string("Falcon-512 + ML-DSA-65 + SLH-DSA-SHA2-128s (SPHINCS+)"));
    json_object_set_new(root, "library", json_string("liboqs 0.12.x"));
    json_object_set_new(root, "note", json_string("All keys are driven by the SPX-QEC ternary distillation pipeline."));
    ...
    free(f_pk); free(f_sk); free(d_pk); free(d_sk); free(s_pk); free(s_sk);
    for (int r = 0; r < ROLE_COUNT; r++) {
    ...
    printf("✅ Complete! All 3 algorithms (Falcon + Dilithium/ML-DSA + SPHINCS+/SLH-DSA) are now working.\n");
    return 0;
}
